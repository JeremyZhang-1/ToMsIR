...
class FrequencyEnhancementModule(nn.Module):
    def __init__(self, channels):
        ...
        # 轻量级权重生成器：动态生成低频、高频、原始特征的融合权重 alpha, beta, gamma
        # 输出 shape 为 (B, 3, 1, 1)，Softmax 保证 alpha + beta + gamma = 1
    ...
    def forward(self, x):
        ...
        # 3. 动态生成注意力权重 alpha, beta, gamma
        weights = self.attention_generator(x)
        alpha = weights[:, 0:1, :, :]
        beta  = weights[:, 1:2, :, :]
        gamma = weights[:, 2:3, :, :]

        # 4. 注意力加权融合
        fused_features = alpha * low_enhanced + beta * high_enhanced + gamma * x

        # 5. 最终输出
        out = self.fusion_adjust(fused_features)
